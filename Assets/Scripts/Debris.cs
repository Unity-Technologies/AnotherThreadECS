...
public class Debris
{
    ...
    public void init(Material material)
    {
        ...
        var colors = new UnityEngine.Color[POINT_MAX*2];
        for (var i = 0; i < POINT_MAX; ++i) {
            colors[i*2+0] = new UnityEngine.Color(0f, 0f, 0f /* not used */, 1f);
            colors[i*2+1] = new UnityEngine.Color(0f, 0f, 0f /* not used */, 0f);
        }
        mesh_ = new Mesh();
        mesh_.name = "debris";
        ...
        material_.SetColor(material_Color, new UnityEngine.Color(1f, 1f, 1f));
    }
    
    public void render(UnityEngine.Camera camera, double render_time)
    {
        if (material_ == null) {
            return;
        }
